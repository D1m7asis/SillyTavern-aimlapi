...
モバイルデバイスにも適したレイアウト・マルチAPI（KoboldAI/CPP、Horde、NovelAI、Ooba、OpenAI、OpenRouter、Claude、Scale、**AI/ML API**）、VN ライクな Waifu モード、Stable Diffusion、TTS、WorldInfo（伝承本）、カスタマイズ可能なUI、自動翻訳、大量のプロンプトオプション＋サードパーティの拡張機能をインストールする機能
...
* [AI/ML API](https://aimlapi.com/app/?utm_source=sillytavern&utm_medium=github&utm_campaign=integration) Deepseek、Gemini、ChatGPT など300以上のモデルを提供し、チャット補完、テキスト補完、画像生成をエンタープライズ級のレート制限と稼働率で利用可能です。
* プロンプト生成フォーマットの調整
